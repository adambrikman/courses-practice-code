...
studentData.forEach(function passStudentData(element) {
  return deepJS.addStudent(element.id, element.name, element.paid);
});

// 6. Define the student enrollments by calling `deepJS.enrollStudent(..)` for each.
var enrolledArr = [ 410, 105, 664, 375 ];

enrolledArr.forEach(function passEnrollData(element) {
  return deepJS.enrollStudent(element); 
});
