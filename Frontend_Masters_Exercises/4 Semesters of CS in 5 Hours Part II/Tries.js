/* To pass the unit tests, you will need to create a function called 
createTrie that accepts a list of strings as a parameter and returns an object 
with a method on it called `complete`. complete is a method that when called 
with a string will return an array of up to length three that are autocompleted 
suggestions of how to finish that string.

For the sake of this exercise, it does not matter which order these strings are 
returned in or if there are more than three possible suggestions, which three you 
choose
...
class Node {
  // In addition to this class, you'll need a constructor and instance variables
  children = [];
  value = "";
  terminus = false;
