...
function pickNumber(v, vals) {
  
  if(!vals.includes(v)) {
    vals = [v, ...vals];
    vals.sort(function asc(a,b) { return a-b });    
  }
...
while (luckyLotteryNumbers.length < 6) {
  /* To preserve function purity for pickNumber, we abstract away both our
     call to lotteryNum() and our updates to luckyLotteryNumbers by passing 
     these as arguments instead of calling/updating them in pickNumber. */
