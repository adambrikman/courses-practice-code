...
function isValidName(name) {
    if(typeof name == "string" && name.replace(/\s/g, "").length > 2) {
        return true;    
    }
    return false;
  }
  ...
  function hoursAttended(attended, length) {
    ...
    if(typeof attended == "string" || typeof length == "string" || typeof attended == "number" || typeof length == "number") {
      ...
      if(numAttended >= 0 && numLength >= 0 && Number.isInteger(numAttended) && Number.isInteger(numLength) && numAttended <= numLength) {
        return true;
      }
